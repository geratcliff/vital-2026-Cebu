...
## Review of CEA ratio {.section-slide}
...
-   A [POPULATION-LEVEL]{.bold .accent-text} decision-making tool
:::

## Cost-Effectiveness Analysis IS NOT

::: {.icon-bullets-x .incremental}
...
::::::: r-vstack
::: {.brown-text data-id="costA"}
Cost of Intervention
:::

::: {.brown-text data-id="costB"}
Cost of Alternative
:::

::: {.secondary-text data-id="benefitA"}
Benefit of Intervention
:::

::: {.secondary-text data-id="benefitB"}
Benefit of Alternative
:::
:::::::
...
- $E_1$: effectiveness of new treatment (e.g., LE, QALYs, DALYs, other outcomes)
...
Strategy: Treat All 
![](images/cea-neurologic-disease_tx.png)
...
![](images/cea-neurologic-disease_notx.png)
...
## Non-Competing vs. Competing CEAs {.section-slide}
...
Decision problem has [non-competing]{.bold .accent-text}
programs/interventions.

-   Each program is compared to a null alternative
-   Calculate an "average" cost-effectiveness ratio.
:::

::: {.card .fragment}
**Competing Choice**

Decision problem has [competing mutually exclusive]{.bold .accent-text}
programs/ interventions for the same purpose.

-   Two or more active alternatives in addition to the null option.
-   Calculate an "incremental cost- effectiveness ratio" to get the
    added cost per unit of added benefit of an option, relative to the
    next less expensive choice
:::
:::::
...
df_ex %>% 
  ...
  scale_x_continuous(limits = c(-max(abs(df_ex$qalys)),max(abs(df_ex$qalys))), 
                     breaks = NULL, position="top") + 
  scale_y_continuous(limits = c(-max(abs(df_ex$cost)),max(abs(df_ex$cost))), 
                     breaks = NULL, position = "right") +
  labs(x = "Incremental Costs", y = "Incremental Health") + 
  geom_dl(method = list("smart.grid",hjust=.9,cex=2,vjust=1),
          aes(label = glue("{label}"))) +
  theme(legend.position = "none", 
        plot.background = element_rect(fill = "#f2eee4", colour = NA),  # Add colour = NA
        panel.background = element_rect(fill = "#f2eee4", colour = NA), # Add this
...
::: {.step-content .incremental .arrow-bullets}
...
::: {.step-content .incremental .arrow-bullets .smaller style="line-height:1.15"}
- Rank other programs in ascending order by their cost-effectiveness ratio (lowest to highest)
- Programs are selected from the **least** to the **most** expensive until the budget is exhausted
- Final selection depends on the **budget constraint**
:::
::::::
:::::::
::::::::
...
::: incremental
-   Initial budget: \$80
-   Budget savings: [\$20]{.accent-text}
-   Total budget: \$80 + [\$20]{.accent-text} = \$100
:::
::::
...
## Shopping Spree Problem {.smaller .table-compact}
...
::: highlight-box
Can do the same with other objectives (e.g., Minimize costs, subject
decision to 'minimum benefit' constraint, etc.)
:::
...
1.  [Programs are mutually exclusive.]{.bold .accent-text}
2.  Different costs & effects associated with each.
3.  [Requires calculation of an *Incremental Cost-Effectiveness Ratio*
    (ICER)]{.bold .accent-text}
:::
:::::
...
## Incremental Cost-Effectiveness Analysis {.section-slide}

::: section-number
03
:::

# A. Incremental CEA in [Pictures]{.accent-text}
...
::: {.content-visual .fragment}
![](images/vis_icer1.png){fig-align="center"}
:::
:::::


## (2a) Identify Dominated Strategies {.smaller auto-animate="true"}
...
## (2b) Identify Dominated Strategies {.smaller auto-animate="true"}
...
## What about strategy B? {auto-animate="true"}
...
:::content-text
::: incremental
-   Suppose it is feasible to partially implement strategies [A]{.bold .secondary-text} and
    [D]{.bold .secondary-text} .
    
:::

::: {.highlight-box .fragment}
For example, we could implement [A]{.bold .secondary-text} for [90%]{.bold .accent} of the population and [D]{.bold .secondary-text} for [10%]{.bold .accent}  of the population, or vice versa.
...
::: {.highlight-box}
For example, we could implement [A]{.bold .secondary-text} for [10%]{.bold .accent} of the population and [D]{.bold .secondary-text} for [90%]{.bold .accent} of the population, or vice versa.
:::
:::
...
::: {.highlight-box}
For example, we could implement [A]{.bold .secondary-text} for [50%]{.bold .accent} of the population and [D]{.bold .secondary-text} for [50%]{.bold .accent} of the population, or vice versa.
:::

:::fragment
[Can we make any statements about [B]{.secondary-text} now?]{.bold}
:::
:::
...
## Extended (Weak) Dominance {auto-animate="true"}
...
::: {.column width="40%"}
::: definition
-   The efficiency frontier is the set of non-dominated strategies: Lower & right = better (more effective and less costly).
-   Any point above the A-D line (like B) delivers fewer QALYs per dollar than the A–D mix.
...
The slope of a line connecting two points is the incremental
    cost-effectiveness ratio comparing those strategies. 
<br><br><br>    
More on this later!
:::
:::
...
# B. Incremental CEA in [Tables]{.accent-text}
...
.highlight-red}[^2] - what are you getting for each additional cost. 
:::
...
## Dominance & extended dominance {.section-slide}
...
-   A steeper slope = the "C" (numerator) is increasing faster than "E" (denominator) = less efficient use of resources
:::
...
df1 %>%
  ...
  select(
    Strategy,
    Cost, DALYs,                 # <-- totals (keep these)
    iCost, DALYs_averted,         # <-- incrementals
  ...
  kable(
    col.names = c(
      "Strategy", "Cost", "DALYs",
      "Incremental Cost", "DALYs Averted",
      "Incremental Cost per DALY Averted", "Status"
    ),
    format.args = list(big.mark = ",")
  ) %>%
  kableExtra::kable_styling(bootstrap_options = "condensed") %>%
  kableExtra::row_spec(color.me.red, bold = TRUE, color = "black", background = "#FCD2CA")
```
:::


:::concept-grid
:::{.concept-box .accent style="font-size: 3em"}
...
-   In LMICs, 0.5-1x per capita gross domestic product (GDP) per DALY
    averted is what is now recommended.
...
-   Decision should be informed by the value of what will be given up as
    a consequence of those cost. Known as the "opportunity cost."
...
[Source: See K Claxton on the estimation of the NICE threshold in the
    UK / Woods et al, & others]{style="font-size:2vh"}
...
[If our CE threshold was 2x GDP (GDP = \$2,500), which option would
    we choose as decision makers?]{.fragment}
    <br><br>
...
::: key-points
- Use average C/E ratios for non-competing “shopping spree” problems; use ICERs for mutually exclusive choices.
- Build the table in order: totals → sort by cost → incrementals and ICERs.
- Remove dominated and extendedly dominated strategies, then re-calculate to obtain the efficiency frontier.
- Choose the best remaining option under the cost-effectiveness threshold with a complete comparator set.
:::
:::
...
::: next-info
Next Lecture: <strong> Markov Models </strong>
:::
