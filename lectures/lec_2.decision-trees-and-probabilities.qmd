...
| ![](images/lec2/rain_beach.png){style="max-height: 5vh; margin-bottom: 0"} At beach, rain [WORSE]{.accent-text} | 0.4 |
:::
:::::
...
![](images/lec2/expected_value.png){width="269"}
...
::: example-text
On average, patients given Treatment A will live 0.30 years (or 3.6 months [=0.30*12]) longer than
...
When the probability of rain is 50% at BOTH the beach and home, given how
we weighted the outcomes, going to the beach would be the same as
staying at home
...
::: {.highlight-box .primary style="font-size:0.95em"}
::: incremental 
- **What would** $p_B$ need to be to yield an expected payoff *at the beach* of 0.74?
  - We just solved for the **indifference threshold** assuming the probability of rain was the same at the beach and at home.
  - Now we **vary rain at the beach** while keeping the probability of rain at home fixed at **30%**.
...
payoffs and solve for $p_B$, **because we are varying the probability of rain at the beach ($p_B$), not the probability of rain at home ($p_H$).**
...
![](images/lec2/vaccine.png)

## Mutually exclusive events

Assuming events are mutually exclusive, then the probability of 2 events
occurring is the <u>sum of the probability of each event occurring individually</u>
...
-   Moving from left to right, the first probabilities in the tree show the probability of an event.
-   Subsequent probabilities are [conditional]{.bold .primary-text}. The probability of an event given that an earlier event did or did not occur.
-   Multiplying probabilities along pathways estimates the pathway probability, which is a [joint]{.bold .primary-text} probability.
:::
...
::::: split-content
::: content-text
[P(A\|B)]{.bold .accent-text}  <br>
= P(death in first year \| mother who smokes) <br>
= 16,712 / (1,197,142 + 16,712) <br>
= 14 per 1,000 births
...
THEN, the conditional probability of P(A\|B) = the probability of an infant death,
...
[P(A and B)]{.bold .accent-text} = 14,442 <br>
[P(B)]{.bold .accent-text}= 3,818,736 <br>
[P(A\|B)]{.bold .accent-text} = 14,442/3,818,736 = 0.0038
...
[P(B\|A)]{.bold .accent-text}<br>
= 14,442 / (14,442+ 21,054) <br>
= 0.41

[4.1 normal birthweights of 1,000 baby deaths within 1 year from birth]{.bold .accent-text}
...
::: {.pros-cons-new}
::: {.pros-column}
<div class="column-title">Strengths</div>
...
::: {.cons-column}
<div class="column-title">Limitations</div>
...
> Each strategy must include all relevant risks *and* benefits.  <br>
...
2. **2 branches versus 3** <br>

> There is nothing inherently wrong with having three (or more) branches, as long as events are mutually exclusive and probabilities sum to 1. <br> 
> However, additional branches increase model complexity and can complicate sensitivity analyses without necessarily adding insight.
...
3. **No embedded decisions** 
<br>

> Each decision tree should address a single decision problem.  <br>
