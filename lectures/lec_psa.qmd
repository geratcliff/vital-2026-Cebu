...
1.  PSA sample size: M = 5
2.  Draw a random number between 0 and 1: `r set.seed(23); round(runif(1),4)`
3.  Feed thhis into the inverse Normal CDF: `NORM.INV(.5766 , 2, 0.1) = 2.019`
4.  Repeat steps 1-3 four more times to get the PSA sample for this parameter.
...
-   Figure plots values at each iteration, the average across 1,000 iterations (large points) and ellipses that capture \~95% of points.[^1]
:::
...
[^1]: Unfortunately, this figure cannot be directly created in Excel; you can use our Shiny app to create it though!
