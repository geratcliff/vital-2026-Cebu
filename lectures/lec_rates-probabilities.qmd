...
**Epidemiologic terminology:** - **Cumulative incidence (risk):** the
probability of experiencing the event by time *t* - Examples: "1-year
risk," "5-year cumulative incidence"
...
-   **Incidence (hazard) rate:** number of events per unit of
    **person-time**
...
::: incremental
-   **Probability (risk):** chance an individual experiences an event by
    a specified time\
...
![](images/paste-8191B0E3.png){.fragment width="784"}
...
-   **Rate (if all 100 followed for 4 years)**\
    Total person-time = $100 \times 4 = 400$ person-years;
    $40/400 = 0.10$ deaths per person-year <br> *Interpretation:* ≈1
    death per 10 years of total follow-up across the study population
    (aggregated person time)
:::
...
-   A standard Markov model assumes an exponential (constant-hazard)
    transition process, so we can easily move between rates
    (instantaneous) and probabilities (over a time period).

-   In other words, the risk per unit time doesn't change over time. If
    the hazard were not constant (say risk increases the longer you're
    sick), you would need a more complex time-dependent model.

-   It is common to use the following formula to convert [rates to
    probabilities]{style="background-color: yellow;"} under a constant
    (exponential) hazard.
...
-   Standard rate/probability conversions\
    $p_{HS}= 1 - e^{-r_{HS}\Delta t}$\
    converts transitions independently, ignoring that death
    competes with sickness within the same time interval.

-   This hides within-cycle events. 

-   Can lead to **underestimation of deaths** with long cycles.
