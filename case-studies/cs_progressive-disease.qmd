...
Suppose there is a new preventive measure that can be undertaken to reduce the likelihood of disease progression. There is also a more expensive treatment option for individuals with progressive disease that reduces the probability of death from disease, and slightly increases their quality of life with the progressive disease.

The Minister of Health has empowered your team to conduct an economic evaluation to investigate the costs and benefits of the status quo (i.e., do nothing) vs. adopting one of the following strategies:
...
2.  Implement the higher-cost treatment strategy. This strategy costs more per person, but it reduces the probability of death from disease and raises quality of life with the disease.
...
+-----------------+------------+------------------------------------------------------------------------------------------------------------------+
| c_progressive   | 3000       | Annual (cycle) cost of progressive disease state.                                                                |
+-----------------+------------+------------------------------------------------------------------------------------------------------------------+
| c_death_disease | 5000       | One-time transition cost of death from progressive disease.                                                      |
+-----------------+------------+------------------------------------------------------------------------------------------------------------------+
| c_prevention    | 500        | Annual (cycle) cost of prevention.                                                                               |
+-----------------+------------+------------------------------------------------------------------------------------------------------------------+
| c_treatment     | 25,000     | One-time cost of progressive disease treatment (occurs at point of transition from mild to progressive disease). |
+-----------------+------------+------------------------------------------------------------------------------------------------------------------+
...
+-----------------------+------------+-------------------------------------------------------------------------------+
| u_progressive         | 0.75       | Quality-of-life (QoL) weight for progressive disease health state.            |
+-----------------------+------------+-------------------------------------------------------------------------------+
| u_progressive_treated | 0.85       | Quality-of-life (QoL) weight for progressive disease health state if treated. |
+-----------------------+------------+-------------------------------------------------------------------------------+
...
Your first objective is to build out a status quo or "do nothing" strategy based on the description and parameters defined above. **This is a strategy in which neither preventive measures nor additional treatments are used.**

To get you going, we have built out the beginnings of a model based on an alive-dead model with age-specific background mortality rates. You can download this model at [this link](case-studies/cs_progressive-disease_blank.amua) TK CHECK LINK. The image below shows the structure of this model:
...
![](images/clipboard-3333040390.png)
...
![](images/clipboard-3583703224.png)
...
1.  Make sure that the life expectancy "payoff" ("LE") is set to 1 for all health states in which an individual remains alive (Healthy, Asymptomatic, Progressive).

2.  Temporarily set the probabilty of disease-related death to 0.

![](images/clipboard-33576193.png){fig-align="center"}
...
::: {.callout-important appearance="minimal"}
Make sure you set the transition probability for disease-related death back to `p_death_disease`!
:::
...
-   Add outcomes for costs and quality-adjusted life expectency (QALE)
...
![](images/clipboard-1019102486.png){fig-align="center" width="500"}
...
-   Next, in the model itself, define the following cycle-specific payoffs based on the values in the tables above. Many of these parameters are already defined for you in the parameters tab in the Amua model:
    ...
    -   Costs and QoL in Asymptomatic state

    -   Costs and QoL in Progressive Disease state

    -   QoL in Death states

-   You will also need to define some one-time costs. For example, death from disease carries a one-time cost (`c_death_disease`). We will add this cost by assigning a one-time cost at the time an individual transitions to the Death (Disease) state from the Progressive disease state. You can add this one-time cost by *right-clicking* on the transition node <img src="../amua-images/arrow_48.png" alt="Custom Icon" width="20" height="20"/> after "Death (Disease)" in the Progressive Disease health state, and then clicking on <img src="../amua-images/cost_24.png" alt="Custom Icon" width="20" height="20"/> Add Cost.

![](images/clipboard-132271593.png){fig-align="center"}

::: {.callout-note appearance="minimal"}
Once you have added all costs and utility outcomes to your model, run the model and verify that it calculates expected costs and QALE for the status quo strategy.
:::
...
![](images/clipboard-4149011598.png){fig-align="center" width="1000"}
...
![](images/clipboard-1448918543.png)

-   Rename the new strategy to "Prevention" and adapt the transition probabilities using the prevention strategy parameters defined in the tables above (and in the Amua model parameters).
...
![](images/clipboard-3868901548.png){fig-align="center" width="800"}
