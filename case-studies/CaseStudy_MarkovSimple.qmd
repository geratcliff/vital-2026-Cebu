...
1. Select the Markov chain labeled "Treat None" 
![](images/simpleMarkov/AddState1.png)
...
Typically within Markov models events are modeled from worst to best. For ease of sensitivity analysis, we recommend only having two branches off each decision node. 
...
4. Nothing can happen after you die so we will change that to a state transition node.

5. Select the "Die" branch and click the "Change to State Transition" ((![](/amua-images/changeTypeTrans_24.png).) button on the top bar. You should get something like this. [It may have auto filled a state or it may also be blank]
...
[Don't worry about the Treat branch, we will come back to it]
...
Once your model matches the one in the dropdown above, you can copy and paste if for the "Not Sick -> Treat" arm. 

1. Select the item you want to copy. In this case it is the "Sick -> Treat" arm of the model. 
...
6. Rename one of the branches to "Not Sick -> Treat" 
