---
title: "Case Study 2 - Intro to Amua"
---
...
-   Make sure that you hit ![](../amua-images/refresh_16.png) **Refresh** once you change the value in a table cell.

-   Click "OK" to add the updates to the model.
...
2.  In the parameter pop-up window, enter a parameter name (Name) and the parameter value ($f(x)$ Expression). The description is optional but useful if the model is being shared with others. ![](images/tree/AddParameterPopUp.png)

3.  Click "Save" to add the parameter

<!-- -->

4.  Add each parameter, one-by-one, with its corresponding value as detailed below:
...
Download the Model Answer: [cs_decision-tree_ANS.amua](cs_decision-tree_ANS.amua)
